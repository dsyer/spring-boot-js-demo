...
HTMX can do more. Instead of an SSE stream, and endpoint can return a regular HTTP response, but compose it as a set of elements to swap on the page. HTMX calls this an "out of band" swap because it involves enhancing content of elements on the page that are not the same one that triggered the download.
...
To migrate the rest of the Stimulus content to React we need to write a new chart chooser. So we can start with a an empty "chart" tab:
...
where `ChartChooser` is a list of buttons:
...
Chart.js isn't actually an ESM module, so you can't import it into a Babel script. We import it in a separate module, and `Chart` has to be defined as a global so we can still use it in our React component.
...
and then we can implement the `<Content/>` as a component that fetches HTML from the `/test` endpoint:
...
Here we have 3 executions: `install-node-and-npm` installs Node.js and NPM locally, `npm-install` runs `npm install` and `npm-build` runs a script to build the Javascript and possibly CSS. We will need a minimal `package.json` to run them all:
...
It's OK to add all the generated code to your `.gitignore` (i.e. `node/`, `node_modules/`, and `package-lock.json`).

### Building with Rollup

The good people who maintain Bootstrap use https://rollupjs.org/guide/en/[Rollup] to bundle their code, so that seems like a valid choice. Feel free to experiment with other tools. To get started with Rollup we will need some development dependencies in `package.json` and a new build script:
...
So if we move all the Javascript into `src/main/js/index.js` we would have this in the `<head>` of `index.html`:
...
We keep the CSS for now, and we can deal with a local build for that later. So in `index.js` we have all the `<script>` tag contents mushed together (or we could have split it up into modules and imported them):
...
Then if we build and run the app it should all work, and Rollup creates a new `index.js` in `target/classes/static` where it will be picked up by the executable JAR. Because of the action of the "resolve" plugin in Rollup, the new `index.js` has all of the code that is needed to run our application. If any dependencies are packaged as a proper ESM bundle, Rollup will be able to "tree shake" the unused parts of them out. This works for Hotwired Stimulus at least, and most of the others get included wholesale, but the result is still only 750K (most of it Bootstrap):
...
The browser only has to download this once, which is an advantage when the server is HTTP 1.1 (HTTP 2 changes things a bit), and it means the executable JAR isn't bloated with stuff that never gets used. There are other plugin options with Rollup to compress the Javascript, but we'll leave it like that for now.

### Building CSS with Sass

So far we have used plain CSS bundled for us in some NPM libraries. Most applications need their own stylesheets and developers prefer to work with some form of templating library, and build time tooling to compile to CSS. The most prevalent such tool (but not the only one) is probably https://www.npmjs.com/package/sass[Sass]. Bootstrap uses it, and indeed packages its source files in the NPM bundle, so you can extend and adapt the Bootstrap styles to your own requirements.
...
Customizations in SCSS would follow that. Then in `index.js` we add imports for this and the Spring utils library:
...
To finish up we can apply the same ideas to the `react-webjars` sample, removing Webjars and extracting Javascript and CSS into separate source files. This way, we can finally get rid of the slightly problematic `@babel/standalone`, for instance. We can start from the `react-webjars` sample and add the Frontend Plugin (or otherwise acquire Node.js), and create a `package.json` either manually or via the `npm` CLI. We will need the React dependencies, and also the build time tooling for Babel. Here's what we end up with:
...
With those build tool configurations in place we can extract all the Javascript from `index.html` and put it in `src/main/resources/static/index.js`. It's almost a copy paste, but we will want to add the CSS imports:
...
You can build that with `npm run build` (or `./mvnw generate-resources`) and it should work.
