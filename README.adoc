...
Chart.js isn't actually an ESM module, so you can't import it into a Babel script. So we import it in a separate module, and `Chart` has to be defined as a global, so we can still use it in our React component.
